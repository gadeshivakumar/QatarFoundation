    ...
    fetch("/api/opportunities", {
      ...
      .then((data) => {
        if (data.message) {
          ...
          loadOpportunities(); // 🔥 refresh from DB
        } else {
...
function loadOpportunities() {
  fetch("/api/opportunities", {
    ...
    .then((data) => {
      ...
      grid.innerHTML = ""; // ✅ now safe (empty container)
  ...
  <div class="opportunity-footer">
    <span class="applicants-count">
        ${opp.max_applicants || 0} applicants
    </span>

    <div class="action-buttons">
        <button class="view-course-btn view-btn">View Details</button>
        <button class="view-course-btn delete-btn" onclick="deleteOpportunity(${opp.id})">
          Delete
      </button>
    </div>
</div>
...
function restorePage() {
  ...
  if (navItem) {
    navItem.click(); // 🔥 triggers existing logic
  }
}
